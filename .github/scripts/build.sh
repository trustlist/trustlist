...
docker build -t unirep-frontend-$env -f packages/frontend/Dockerfile .
...
docker build -t unirep-relayer-$env -f apps/relayer/Dockerfile .
...
docker build -t unirep-node-$env -f apps/node/Dockerfile .
