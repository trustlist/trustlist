...
export default observer(({ listing, setShowDetail }: Props) => {
    ...
    const acceptOfferAlert = (newData: any) =>
        toast.promise(
            async () => {
                ...
                await user.requestData(
                    { [1]: 1 << 23 },
                    memberKeys.indexOf(listing.posterId),
                    newData.responderId
                )
            },
            {
                ...
                success: {
                    render: (
                        <div className="flex space-around gap-3">
                            <div>
                                <div>
                                    Offer accepted! Your contact info will be
                                    shown to this member to enable your offline
                                    transaction.
                                </div>
                                <div>
            ...
            <div className="centered">
                <div className="modal">
                    <div className="detail-content">
                            ...
                            <>
                                <button
                                    ...
                                    onClick={() => {
                                        setShowDetail(false)
                                        navigate(
                                            `/offers/${listing._id}/${listing.title}`
                                        )
                                    }}
                                >
                                    MAKE OFFER
                        ...
                        {memberKeys.includes(listing.posterId) ? (
                            <button className="font-extrabold text-base text-red-500 border-1 border-red-500 px-3 py-2 mb-4">
                                MY LISTING
                            </button>
                        ) : null}
                        {listing.epoch !=
                            ...
                            <div className="pt-6">
                                {trustScoreKeys.map((key) => {
                                    const matchingEntry = Object.entries(
                                        ...
                                        ([scoreName]) => scoreName === key
                                    )[0]
                                    const revealed = matchingEntry !== undefined
                                    const initiated = matchingEntry
                                        ? Number(matchingEntry[1]) >> 23
                                        : 0
                            ...
                            <div className="offer-scroll">
                                {offers && offers.length > 0
                                    ? offers.map((offer: Offer) => {
                                          const responderScores = JSON.parse(
                                              offer.scoreString
                                          )
                                          return (
                                              <div
