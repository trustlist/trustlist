...
export default observer(({ setShowMemberDash }: Props) => {
    ...
    const transitionSuccess = () =>
        toast.success(
            'Transition successful! Your provable data has been updated and you can now participate in the current epoch.'
        )
    const transitionFailed = () =>
        toast.error('Failed to transition to the current epoch.')
                ...
                <div className="modal">
                    <div className="dash-content">
                        <div className="stats-container">
                            <div>
                                <div>
                                    <h3 className="py-3 text-lg font-semibold">
                                        my latest trust scores:
                                    </h3>
                                </div>
                                {app.scoreDescriptions.map((desc, i) => {
                                    const initiated = user.data[i]
                                        ? Number(user.data[i] >> BigInt(23))
                                        : 0
                                    const completed = user.data[i]
                            ...
                            <div>
                                <div>
                                    <h3 className="pt-8 pb-3 text-lg font-semibold">
                                        my provable trust scores:
                                    </h3>
                                </div>
                                {app.scoreDescriptions.map((desc, i) => {
                                    const initiated = user.provableData[i]
                                        ? Number(
                                              user.provableData[i] >> BigInt(23)
                                          )
                                        : 0
                                    const completed = user.provableData[i]
                                        ? Number(
