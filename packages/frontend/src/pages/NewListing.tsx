...
const FormSteps: FormStep[] = [
    {
        id: 'select-category',
        description: 'Choose the categories for your listing',
        fields: ['categories'],
    },
    {
        id: 'general-info',
    ...
    {
        id: 'trust-scores',
        description: 'Choose which trust scores to show',
        fields: ['revealTrustScores'],
    },
]
...
type FormFooterAndHeaderProps = {
    currentStep: number
    changeStep: React.Dispatch<React.SetStateAction<FormStep>>
    trigger: UseFormTrigger<ListingFormValues>
}

const createInitialCategories = (
    sectionsWithCategories: Record<string, string[]>
) => {
    // create an empty array for each section in listing type
    return Object.keys(sectionsWithCategories).reduce<Record<string, string>>(
        (acc, key) => {
            acc[key] = ''
            return acc
        },
                ...
                render={({ field }) => (
                    <FormItem>
                        <FormLabel className="text-base flex gap-2 items-center">
                            Contact
                            <Tooltip
                                text="Please include a Telegram or Discord handle. Your contact information will only be shown to the member whose offer you accept."
                                content={
                                    <img
...
type TrustScoreFormStepProps = StepSectionProps & {
    trustScores: Record<TrustScoreKey, TrustScoreInfo>
}

const TrustScoreFormStep = ({
    control,
    trustScores: trustScoresFromData,
}: TrustScoreFormStepProps) => {
    ...
    return (
        <section className="flex flex-col space-y-4">
            {Object.entries(trustScoresFromData).map(([key, scoreInfo]) => {
                ...
                const initiated = user.provableData[scoreInfo.index]
                    ? Number(user.provableData[scoreInfo.index] >> BigInt(23))
                    : 0
        ...
        <section className="py-3">
            ...
            >
                {currentStep > 1 && (
                    <button
                        type="button"
                        className="px-2 py-1 border-muted-foreground text-muted-foreground"
...
const NewListingPage = () => {
    ...
    const [trustScoresFromData, setTrustScoresFromData] = useState({
        ...trustScores,
    }) // Make copy we can use
    ...
    const trustScoreKeys = Object.keys(
        TrustScoreKeyEnum
    ) as (keyof typeof TrustScoreKeyEnum)[]
    ...
    const transitionAlert = () =>
        toast.promise(
            ...
            },
            {
                ...
                success:
                    'Transition successful!  Please confirm whether you would like your updated scores to be shown and click publish to complete your listing.',
    ...
    const getEpochAndKey = async () => {
        ...
        return {
            ...
            userEpochKey: posterId,
            nonce: epkNonce,
        }
    }

    const generateScores = (scoresRevealed: Record<TrustScoreKey, boolean>) => {
        return Object.entries(scoresRevealed).reduce(
            (newScores, [scoreKey, isRevealed]) => {
                if (isRevealed) {
                    return {
                        ...newScores,
                        [scoreKey as TrustScoreKey]:
                            trustScoresFromData[scoreKey as TrustScoreKey]
                                .score,
                    }
                }
                return newScores
    ...
    const publishingAlert = (newData: any) =>
        toast.promise(
            async () => {
                await createNewListing(newData)
                // +1 to current member's initiated LP score
                await user.requestData({ [0]: 1 << 23 }, newData.nonce, '')
            },
            {
                pending:
                    'Please wait a moment while your listing is being published...',
                success: {
                    render: (
                        <div className="flex space-around gap-3">
                            <div>
                                <div>
                                    Listing published! One "initiated" point has
                                    been added to your Legitimate Posting score.
                                </div>
                                <div>
                                    Please complete your deal during this epoch
                                    to build your LP reputation.
                                </div>
                            </div>
                            <button
    ...
    switch (step.id) {
        ...
        default:
            content = (
                <div>Wait! This isn't a step... how did you get here?</div>
            )
    }
    return (
        <Form {...listForm}>
            <form
                onSubmit={listForm.handleSubmit(publishPost, onFormError)}
                className="flex flex-col p-3 justify-center container py-6 space-y-3 max-w-3xl text-foreground"
            >
                <FormHeader
                    changeStep={changeStep}
                    currentStep={currentStepNumber}
                    trigger={listForm.trigger}
                />
                {content}
                <FormFooter
                    currentStep={currentStepNumber}
